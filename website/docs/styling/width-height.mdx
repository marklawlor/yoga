...
Width and height support maths expressions like `calc()`, `min()`, `max()`, and `clamp()`.
See [Maths Expressions](/docs/styling/math-expressions) for the full API reference and examples.
